  ...
  class ar_admin_login {
    ...
    function canPerform($user_id, $user_name) {
      if (DB_TYPE == 'mysql') {
        ...
        $check_query = tep_db_query("select id from " . TABLE_ACTION_RECORDER . " where module = '" . tep_db_input($this->code) . "' and (" . (!empty($user_name) ? "user_name = '" . tep_db_input($user_name) . "' or " : "") . " identifier = '" . tep_db_input($this->identifier) . "') and date_added >= (now() - INTERVAL '" . (int)$this->minutes  . " minutes') and success = '0' order by date_added desc limit " . (int)$this->attempts);
      }
