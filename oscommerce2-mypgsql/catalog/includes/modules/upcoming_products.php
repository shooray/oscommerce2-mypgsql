...
*/
  ...
  if(DB_TYPE == 'mysql') {
    ...
    $expected_query = tep_db_query("select p.products_id, pd.products_name, products_date_available as date_expected from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where (CAST(products_date_available AS date) - CAST('0001-01-01' AS date) + 366) >= (CURRENT_DATE - CAST('0001-01-01' AS date) + 366) and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by " . EXPECTED_PRODUCTS_FIELD . " " . EXPECTED_PRODUCTS_SORT . " limit " . MAX_DISPLAY_UPCOMING_PRODUCTS);
  }
