  ...
  if (DISPLAY_PRICE_WITH_TAX == 'true') {
    if ($pfrom > 0) {
      if (DB_TYPE == 'mysql') {
        ...
        $where_str .= " and ((case when s.status = 1 then s.specials_new_products_price else p.products_price end) * (case when gz.geo_zone_id is null then 1 else (1 + (tr.tax_rate / 100)) end) >= " . (double)$pfrom . ")";
      }
    }
    if ($pto > 0) {
      if (DB_TYPE == 'mysql') {
        ...
        $where_str .= " and ((case when s.status = 1 then s.specials_new_products_price else p.products_price end) * (case when gz.geo_zone_id is null then 1 else (1 + (tr.tax_rate / 100)) end) <= " . (double)$pto . ")";
      }
    }
  } else {
    if ($pfrom > 0) {
      if (DB_TYPE == 'mysql') {
        ...
        $where_str .= " and ((case when s.status = 1 then s.specials_new_products_price else p.products_price end) >= " . (double)$pfrom . ")";
      }
    }
    if ($pto > 0) {
      if (DB_TYPE == 'mysql') {
        ...
        $where_str .= " and ((case when s.status = 1 then s.specials_new_products_price else p.products_price end) <= " . (double)$pto . ")";
      }
    }
  }
