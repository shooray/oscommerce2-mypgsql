...
<!--
    ...
    if (http.readyState == 4) {
      if (http.status == 200) {
        ...
        if (result[0] == '1') {
          ...
          loadXMLDoc("rpc.php?action=dbImport&dbType=" + urlEncode(dbType) + "&dbPort=" + urlEncode(dbPort) + "&server=" + urlEncode(dbServer) + "&username=" + urlEncode(dbUsername) + "&password=" + urlEncode(dbPassword) + "&name=" + urlEncode(dbName), handleHttpResponse_DoImport);
        } else {
  ...
  function prepareDB() {
    ...
    dbType = document.getElementById("DB_TYPE").value;
    dbPort = document.getElementById("DB_PORT").value;

    loadXMLDoc("rpc.php?action=dbCheck&dbType=" + urlEncode(dbType) + "&dbPort=" + urlEncode(dbPort) + "&server=" + urlEncode(dbServer) + "&username=" + urlEncode(dbUsername) + "&password=" + urlEncode(dbPassword) + "&name=" + urlEncode(dbName), handleHttpResponse);
  }
  
  function showDefaultDbPort() {
  	if(document.getElementById("DB_TYPE").value == "mysql") {
  		document.getElementById("DB_PORT").value = 3306;
  	} else if(document.getElementById("DB_TYPE").value == "pgsql") {
  ...
  <div class="contentPane">
    ...
    <table border="0" width="99%" cellspacing="0" cellpadding="5" class="inputForm">
      <tr>
        <td class="inputField">
        	Select Database<br />
        	<select id="DB_TYPE" name="DB_TYPE" class="text" onchange="showDefaultDbPort();"><option value="mysql">MySQL</option><option value="pgsql">PostgreSQL</option></select>
        </td>
        <td class="inputDescription">The type of the database, MySQL or PostgerSQL.</td>
      </tr>
      <tr>
      ...
      <tr>
        <td class="inputField"><?php echo 'Server Port<br />' . osc_draw_input_field('DB_PORT', '3306', 'class="text"'); ?></td>
        <td class="inputDescription">The port of the database server.</td>
      </tr>
      <tr>
