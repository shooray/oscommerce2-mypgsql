...
*/
  ...
  $dbType = trim($HTTP_POST_VARS['DB_TYPE']);
  $dbPort = trim($HTTP_POST_VARS['DB_PORT']);
  
  if($dbType == "mysql") {
    osc_db_connect(trim($HTTP_POST_VARS['DB_SERVER']), trim($HTTP_POST_VARS['DB_SERVER_USERNAME']), trim($HTTP_POST_VARS['DB_SERVER_PASSWORD']), trim($HTTP_POST_VARS['DB_DATABASE']), $dbType, $dbPort);
    osc_db_select_db(trim($HTTP_POST_VARS['DB_DATABASE']));
  } else if($dbType == "pgsql") {
    osc_db_connect(trim($HTTP_POST_VARS['DB_SERVER']), trim($HTTP_POST_VARS['DB_SERVER_USERNAME']), trim($HTTP_POST_VARS['DB_SERVER_PASSWORD']), trim($HTTP_POST_VARS['DB_DATABASE']), $dbType, $dbPort);
  }

  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . trim($HTTP_POST_VARS['CFG_STORE_NAME']) . '\' where configuration_key = \'STORE_NAME\'', $dbType);
  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_NAME']) . '\' where configuration_key = \'STORE_OWNER\'', $dbType);
  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_EMAIL_ADDRESS']) . '\' where configuration_key = \'STORE_OWNER_EMAIL_ADDRESS\'', $dbType);

  if (!empty($HTTP_POST_VARS['CFG_STORE_OWNER_NAME']) && !empty($HTTP_POST_VARS['CFG_STORE_OWNER_EMAIL_ADDRESS'])) {
  	osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'""' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_NAME']) . '"" <' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_EMAIL_ADDRESS']) . '>\' where configuration_key = \'EMAIL_FROM\'', $dbType);
  } else {
  	osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_EMAIL_ADDRESS']) . '\' where configuration_key = \'EMAIL_FROM\'', $dbType);
  }

  $check_query = osc_db_query('select user_name from ' . TABLE_ADMINISTRATORS . ' where user_name = \'' . trim($HTTP_POST_VARS['CFG_ADMINISTRATOR_USERNAME']) . '\'', $dbType);

  if (osc_db_num_rows($check_query, $dbType)) {
    osc_db_query('update ' . TABLE_ADMINISTRATORS . ' set user_password = \'' . osc_encrypt_string(trim($HTTP_POST_VARS['CFG_ADMINISTRATOR_PASSWORD'])) . '\' where user_name = \'' . trim($HTTP_POST_VARS['CFG_ADMINISTRATOR_USERNAME']) . '\'', $dbType);
  } else {
    osc_db_query('insert into ' . TABLE_ADMINISTRATORS . ' (user_name, user_password) values (\'' . trim($HTTP_POST_VARS['CFG_ADMINISTRATOR_USERNAME']) . '\', \'' . osc_encrypt_string(trim($HTTP_POST_VARS['CFG_ADMINISTRATOR_PASSWORD'])) . '\')', $dbType);
  }

  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . trim($HTTP_POST_VARS['CFG_STORE_OWNER_EMAIL_ADDRESS']) . '\' where configuration_key = \'MODULE_PAYMENT_PAYPAL_EXPRESS_SELLER_ACCOUNT\'', $dbType);
?>
...
<?php
  ...
  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . $dir_fs_document_root . 'includes/work/\' where configuration_key = \'DIR_FS_CACHE\'', $dbType);
  osc_db_query('update ' . TABLE_CONFIGURATION . ' set configuration_value = \'' . $dir_fs_document_root . 'includes/work/\' where configuration_key = \'SESSION_WRITE_DIRECTORY\'', $dbType);
