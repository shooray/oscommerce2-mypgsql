...
*/
  ...
  if (DB_TYPE == 'mysql') {
    ...
    tep_db_query("update " . TABLE_PRODUCTS . " set products_date_available = NULL where (CURRENT_DATE - CAST('0001-01-01' AS date) + 366) > (CAST(products_date_available AS date) - CAST('0001-01-01' AS date) + 366)");
  }
...
<?php
  if (DB_TYPE == 'mysql') {
    ...
    $products_query_raw = "select pd.products_id, pd.products_name, p.products_date_available from " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS . " p where p.products_id = pd.products_id and p.products_date_available != NULL and pd.language_id = '" . (int)$languages_id . "' order by p.products_date_available DESC";
  }
